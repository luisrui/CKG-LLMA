...
print('loading data...')
iteminfo = pd.read_csv('./Datasets/AmazonBook/books_data.csv')
rateinfo = pd.read_csv('./Datasets/AmazonBook/Books_rating.csv')

name = 'sub'

### Filter for positive reviews(Changing the logic for filtering the reviews)
sep = 4
filtered_reviews = rateinfo[rateinfo['review/score'] >= sep]
...
for linum in tqdm(range(filtered_reviews.shape[0])):
    user = filtered_reviews.iloc[linum]['User_id']
    if pd.isna(user):
        continue
    item = filtered_reviews.iloc[linum]['Id']
    title = filtered_reviews.iloc[linum]['Title']
    bookname_map[title].add(item)
    entity_text.append([user, 'liked', item])
    item_user_map[item].add(user) ## User-User Relations
user_list = pd.unique(filtered_reviews['User_id'])
item_list = pd.unique(filtered_reviews['Id'])
...
# with open(f'./Datasets/AmazonBook/entity2id_{name}.json', 'w') as f:
#     json.dump(entity_map, f)
with open(f'./Datasets/AmazonBook/entity2id.json', 'r') as f:
    entity2id = json.load(f)

## Make Relation Maping
with open(f'./Datasets/AmazonBook/relation2id.json', 'r') as f:
    relation2id = json.load(f)
# with open(f'./Datasets/AmazonBook/relation2id_{name}.json', 'w') as f:
...
mapped_triples.to_csv(f'./Datasets/AmazonBook/triples.csv', index=False)
