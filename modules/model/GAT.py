...
class GAT(nn.Module):
    ...
    def forward_relation_general(self, item_embs, entity_embs, w_r, adj):
        x = F.dropout(item_embs, self.dropout, training=self.training)
        y = F.dropout(entity_embs, self.dropout, training=self.training)
        x = self.layer.forward_relation_general(x, y, w_r, adj)
        x = F.dropout(x, self.dropout, training=self.training)
        return x

    def fusion_item_embs(self, item_embs, i2e_embs, adj_i2a):
        x = self.layer.fusion_item_embs(item_embs, i2e_embs, adj_i2a)
        return x
    
    def fusion_attr_embs(self, attr_embs, a2i_embs, adj_a2i):
        x = self.layer.fusion_attr_embs(attr_embs, a2i_embs, adj_a2i)
        return x
...
class GraphAttentionLayer(nn.Module):
    def __init__(self, in_features, out_features, num_rels, dropout, alpha, concat=True):
        ...
        self.W_h = nn.Parameter(torch.empty(size=(in_features, out_features)))
        nn.init.xavier_uniform_(self.W_h.data, gain=1.414)
        self.a_h = nn.Parameter(torch.empty(size=(2*out_features, 1)))
        nn.init.xavier_uniform_(self.a_h.data, gain=1.414)
        self.W_t = nn.Parameter(torch.empty(size=(in_features, out_features)))
        nn.init.xavier_uniform_(self.W_t.data, gain=1.414)
        self.a_t = nn.Parameter(torch.empty(size=(2*out_features, 1)))
        nn.init.xavier_uniform_(self.a_t.data, gain=1.414)
        ...
        # self.cross_attention_tail = CrossAttentionLayer(self.in_features, num_heads=8)
    ...
    def forward_relation_general(self, item_embs, i2e_embs, ent_embs, e2i_embs, rel_embs, adj_i2a, adj_a2i):
        '''
        item_embs: N_item, dim
        i2e_embs: N_item, e_num, dim
        ent_embs: N_ent, dim
        e2i_embs: N_ent, i_num, dim
        rel_embs: N_item, e_num, dim
        adj_i2a: N_item, e_num
        adj_a2i: N_ent, i_num
        '''
        # N, e_num, dim
        # Wi = item_embs.unsqueeze(1).expand(entity_embs.size())
        
        # #ent_conv = self.cross_attention_tail(query=Wh, key=rel_embs, value=entity_embs, mask=adj)

        # a_input = torch.cat((Wh,ent_conv),dim=-1) # (N, e_num, 2*dim)
        # # N,e,2dim -> N,e,dim
    ...
    def fusion_item_embs(self, item_embs, attr_embs, adj_i2a):
        '''
        item_embs: N, dim
        attr_embs: N, e_num, dim
        adj: N, e_num
        '''
        Wh = torch.mm(item_embs, self.W_h)
        We = torch.matmul(attr_embs, self.W_h)
        Wh = Wh.unsqueeze(1).expand(We.size())
        a_input = torch.cat((Wh, We), dim=-1)
        e = self.leakyrelu(torch.matmul(a_input, self.a_h).squeeze(2))
        zero_vec = -9e15*torch.ones_like(e)
        attention = torch.where(adj_i2a > 0, e, zero_vec)
        attention = F.softmax(attention, dim=1)
        attention = F.dropout(attention, self.dropout, training=self.training)
        attr_emb_weighted = torch.bmm(attention.unsqueeze(1), attr_embs).squeeze()
        h_prime = attr_emb_weighted + item_embs
        if self.concat:
            return F.elu(h_prime)
        else:
            return h_prime

    def fusion_attr_embs(self, attr_embs, item_embs, adj_a2i):
        '''
        attr_embs: N, dim
        item_embs: N, e_num, dim
        adj: N, e_num
        '''
        We = torch.mm(attr_embs, self.W_t)
        Wh = torch.matmul(item_embs, self.W_t)
        We = We.unsqueeze(1).expand(Wh.size())
        a_input = torch.cat((Wh, We), dim=-1)
        e = self.leakyrelu(torch.matmul(a_input, self.a_t).squeeze(2))
        zero_vec = -9e15*torch.ones_like(e)
        attention = torch.where(adj_a2i > 0, e, zero_vec)
        attention = F.softmax(attention, dim=1)
        attention = F.dropout(attention, self.dropout, training=self.training)
        item_emb_weighted = torch.bmm(attention.unsqueeze(1), item_embs).squeeze()
        h_prime = item_emb_weighted + attr_embs
        if self.concat:
            return F.elu(h_prime)
        else:
            return h_prime
